...
class DealsMaker(threading.Thread):
    ...
    def new_order(self, op, code, num, price):
        ...
        print('用户下单，方向：%s，股票代码：%s，委托量：%7s，价格：%7s'
    ...
    def do_deals_matching(self):
        # active_orders => order_id <-> order_ptr
        # 将买单和卖单分别处理
...
class SimuTickReceiver(threading.Thread):
    ...
    def run(self):
        ticks_file = os.path.join(sys.path[0], 'tick_data')
...
def main():
    ...
    while True:
        ...
        if cmd.startswith('order: '):
            orders = cmd[7:].split(',')
            the_dm.new_order(orders[0], code=orders[1], num=orders[2], price=orders[3])
        elif cmd == "exit":
            break
    the_dm.stop_dm()
